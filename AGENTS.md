...
- `app/ai/prompts.py` owns the centralized `word-learning-aids-v1` batch prompt; it requires the model to return exactly the input word multiset and forbids invented etymology, roots, historical origins, homophones, and unverifiable morphology.
- `scripts/generate_word_learning_aids.py` is a resumable DeepSeek batch generator. It reads only the two source CSVs (never the database), requests batches (`--batch-size`, default 20), validates every returned word against the source and the artifact contract, retries structural and transport failures with exponential backoff, isolates failing words, and checkpoints validated records keyed by word under the ignored `build/word_learning_aids/`. It promotes to `data/word_learning_aids.jsonl` only after a full offline validation passes, and writes `data/word_learning_aids.provenance.json` plus a deterministic sampled spot-check report. The DeepSeek API key is read only from `DEEPSEEK_API_KEY` (local `.env`/environment) and is never written to any output.
- `scripts/validate_word_learning_aids.py` is an independent, offline, repeatable strict validator with `--require-complete`; it cross-checks the JSONL against both CSVs and the full contract, exits non-zero on any error, and reports total, level/source/example-origin counts, empty word-family counts, and an error summary.
- `app/db/learning_aid_seed.py` reads and validates the whole JSONL before any write, matches records to existing `Word` rows by headword (never creating a word), and performs an idempotent content-hash-aware upsert. A missing file is a safe no-op; a malformed file is rejected. `app/bootstrap.py` invokes it inside the serialized startup transaction.
- `app/services/learning_aid_view.py` deterministically maps stored JSON into bounded display strings (`英文搭配｜中文义`, `单词 (词性)｜中文义`) and resolves examples so curated words keep `Word.example` while open words use the validated aid example. `review_service.py` and `wordbook_service.py` consume it; the review card, review page, and wordbook page only present the resulting strings and never parse raw JSON, query SQLAlchemy, or call a model adapter.
...
|---|---:|---:|
| Full pytest suite | 266 passed in 14.47s | 2026-08-23 |
| Word learning-aid pipeline | 53 new deterministic tests passed with a fake provider (no network): schema/prompt contracts, batch alignment (missing/extra/duplicate words), structural and transport retries, per-word isolation, checkpoint resume, force-word re-query, atomic promotion blocking, provenance without secrets, offline validator boundary categories, idempotent hash-aware import, migration v8, service/UI mapping, and empty-aid placeholder degradation | 2026-08-23 |
...
| Ruff format gate | 100 files already formatted | 2026-08-22 |
| Mypy application/scripts check | exit code 0; 63 source files | 2026-08-22 |
...
| Offscreen startup/shutdown smoke | latest schema-v8 run exited 0 through the real deferred-close path (`learning_aids=0` with no generated artifact present); prior 20-run lifecycle baseline, post-workspace repetitions, and subprocess regression also passed | 2026-08-23 |
| SQLite integrity and foreign keys | current source database upgraded to schema v7; `favorite_words` exists; `integrity_check=ok`; `foreign_key_check` returned 0 violations. The prior schema-v5 audit recorded 4,611 runtime words (3,329 CET4 + 1,282 CET6), no invalid reviewed-without-time FSRS rows, or stale review leases | 2026-08-22 |
...
- No reproducible functional defect is open in the current source tree. The phase/progress, keyboard guard, one-step undo, favorite wordbook, learning-aid shell, and sidebar-theme changes have automated and offscreen coverage; visible Windows acceptance of these recent UX passes has not yet been recorded. Earlier source/local-AI/frozen/installer checks and the user's prior visible acceptance remain valid evidence for the behavior they covered, but do not substitute for testing the new controls and layout.
...
- Fixed collocations and word-family/derivative content are not generated yet. The full pipeline is implemented and deterministically tested: centralized prompts, bounded strict schemas, a resumable DeepSeek batch generator with per-word checkpointing, an independent offline validator, and a validated idempotent database import. The review UI owns the post-answer presentation boundary and honestly renders a pending state until real validated content exists.
- Actual content generation has not run because no DeepSeek API key is configured on this machine (no `DEEPSEEK_API_KEY` in the environment or a local `.env`). Once a key is present, run `python scripts/generate_word_learning_aids.py` (optionally `--max-items 20` for a trial run) and then `python scripts/validate_word_learning_aids.py --require-complete`. The `docs/prompts/DEEPSEEK_WORD_LEARNING_AIDS_PROMPT.md` handoff specification remains the authoritative contract; it preserves the 13 curated examples, targets new examples for the 4,598 open rows, writes AI material to a separate JSONL/provenance layer instead of changing the hash-pinned source CSV, and requires resumable batches plus complete offline validation before promotion. All generated material is `ai_generated_unreviewed` and must not be described as human-reviewed.
