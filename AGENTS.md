...
- `app/db/models.py` defines Word, LearningState, ReviewLog, FavoriteWord, ConfusionEdge, AIAnalysis, EmbeddingCache, StudyLevelActivation, ReminderRuntimeState, and ReminderReviewLease.
...
|---|---:|---:|
| Full pytest suite | 213 passed in 7.22s | 2026-08-22 |
...
| Schema migration matrix | 10 focused tests passed: fresh, pre-version adoption, v1→v2 FSRS mapping, v2→v3 level activation, v3→v4 review leases, v4→v5 demo-state repair plus successful four-choice rating persistence, v5→v6 undo snapshots, v6→v7 favorite wordbook, idempotency/newer-version guard, transactional rollback | 2026-08-22 |
...
| Offscreen startup/shutdown smoke | latest schema-v7 run exited 0 through the real deferred-close path; prior 20-run lifecycle baseline, post-workspace repetitions, and subprocess regression also passed | 2026-08-22 |
...
- Fixed collocations and word-family/derivative content are not generated yet. The review UI now owns their post-answer presentation boundary and honestly renders a pending state. A future generator must use centralized prompts, bounded strict schemas, per-word/provider/prompt-version cache identity, no whole-database access, and local validation that rejects unrelated headwords, duplicate entries, ungrounded etymology, and oversized text before persistence or display.
- The execution-ready DeepSeek handoff specification for that future generator is `docs/prompts/DEEPSEEK_WORD_LEARNING_AIDS_PROMPT.md`. It preserves the 13 curated examples, targets new examples for the 4,598 open rows, writes AI material to a separate JSONL/provenance layer instead of changing the hash-pinned source CSV, and requires resumable batches plus complete offline validation before promotion.
