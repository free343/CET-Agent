...
- `app/db/models.py` defines Word, LearningState, ReviewLog, PracticeLog, WordAcquisitionState, AcquisitionAttempt, FavoriteWord, MasteredWord, WordLearningAid, WordLearningAidFeedback, ConfusionEdge, AIAnalysis, EmbeddingCache, StudyLevelActivation, ReminderRuntimeState, and ReminderReviewLease.
...
- `app/services/practice_service.py` selects only previously learned, non-mastered cards for four deterministic scopes: yesterday's real reviews in the local calendar, most recently learned, aggregate wrong cards, or learned favorites. It pages with explicit completed-word exclusion and writes a bounded `PracticeLog`; it never calls `schedule_review`, updates LearningState/ReviewLog, changes due time or counters, feeds the confusion graph, or changes reminder state.
...
- `app/ui/acquisition_page.py` owns the persistent three-stage round-robin new-word workflow: level 0 Chinese-meaning choice, level 1 translated cloze English choice, and level 2 spelling/direct confirmation. It never creates formal ReviewLogs, keeps unfinished cards at the current group's tail, reloads the next projection after every persisted attempt, and schedules the next group/voluntary release only after the current group is exhausted. `app/ui/review_page.py` remains the formal-review/free-practice workspace. Both cards expose asynchronous favorite and “完全掌握” actions beside each other; mastered cards disappear only after the marker transaction succeeds. “已掌握单词” is a separate asynchronous recovery page. Formal review retains recognition/active recall followed by Again/Hard/Good/Easy; free practice offers a scope selector and only “没想起/想起来”, persists a PracticeLog, excludes completed words when paging, and has no undo because no FSRS mutation occurred. All study pages keep cumulative progress, bounded learning-aid display, safe answer visibility, assistant-worker tracking, and deferred-close participation.
...
- The sidebar exposes separate “学习新词”, “到期复习”, “自由复习”, “收藏单词”, and “已掌握单词” entries. Entering any study workspace dismisses a visible reminder banner and publishes the shared active-study lease; the close coordinator tracks acquisition/formal/practice page workers, the mastered recovery worker, and all embedded assistant workers.
- `app/ui/dashboard_page.py` shows new count, learned-due count, completed today, seven-day accuracy, streak, frequent wrong words, and any future-clock warning. Statistics load off the GUI thread; refresh requests arriving during a load are coalesced into one follow-up refresh.
...
- `packaging/CET-Agent.spec` bundles the formal JSONL and provenance files as read-only resources.
...
|---|---:|---:|
| Full pytest suite | 353 passed in 45.27s | 2026-08-23 |
...
| Python dependency integrity | `pip check` reported no broken requirements | 2026-08-23 |
| Ruff static check | all checks passed across app/scripts/tests/main | 2026-08-23 |
| Ruff format gate | 130 files already formatted | 2026-08-23 |
| Mypy application/scripts check | exit code 0; 80 source files | 2026-08-23 |
...
| Deterministic chat routing | regression cases cover local vocabulary/grammar/distinction including `混淆`, Unicode normalization, advanced confirmation, empty/general/off-topic refusal, English word boundaries, guarded scope override, and zero-call refusal | 2026-08-23 |
...
| Schema migration matrix | 15 focused tests passed: fresh, pre-version adoption, v1→v2 FSRS mapping, v2→v3 level activation, v3→v4 review leases, v4→v5 demo-state repair plus successful four-choice rating persistence, v5→v6 undo snapshots, v6→v7 favorite wordbook, v7→v8 word learning aids, v8→v9 local learning-aid feedback, v9→v10 optional practice logs, v10→v11 acquisition/mastery tables, idempotency/newer-version guard, transactional rollback | 2026-08-23 |
...
| Offscreen startup/shutdown smoke | latest source-mode run with the enabled independent DeepSeek advanced configuration exited 0 through the real deferred-close path at schema v11; the earlier isolated fresh run imported 4,611 words/aids, created 4,611 acquisition states, and constructed acquisition/formal/practice/mastered workspaces successfully | 2026-08-23 |
...
| SQLite integrity, route readiness, and foreign keys | the retained source database was previously inspected at schema v10 with 4,611 words/aids, 0 feedback/practice rows, `integrity_check=ok`, and 0 foreign-key violations; the latest real offscreen startup upgraded it successfully to v11. Current CET4 projections previously reported 0 untouched due, 1 learned due, 45 uniquely retrievable yesterday words across 30+15 pages, and 20 future-clock ReviewLogs surfaced by the warning path | 2026-08-23 |
...
- No reproducible source defect is open after separating untouched learning, learned-due review, and non-scheduling practice; correcting the learning-aid empty-state ambiguity and narrow-window content clipping/status wrapping; repairing the embedded distinction action's pre-Provider routing refusal; and eliminating cross-word embedded-assistant history leakage. The complete 4,611-record artifact passed the deterministic validator and was imported successfully; semantic quality remains an AI-generated-content review concern rather than a persistence or scheduling defect.
- The current local database contains 20 real ReviewLogs more than five minutes ahead of the observed system clock, caused by a prior clock jump/rollback. Source mode now surfaces this state on the dashboard and excludes those events from current-time metrics and practice selection, but intentionally does not rewrite user history. The user should keep Windows time/timezone synchronized; a future explicit repair tool would require separate design and confirmation.
...
- `qwen2.5:3b` remains a small local model. The default compact “怎么记” action no longer uses it, while simple example explanations remain acceptable; open-ended deep grammar/usage/distinction answers may still be shallow. The independent DeepSeek advanced Provider is now explicitly enabled on this workstation and available through every chat selector, but its generative lexical/usage advice remains unreviewed model output rather than authoritative word-card content.
...
- The last distributable onedir package/installer remains historical and predates the current split study routes, wordbook, generated-aid, schema-v9 feedback, schema-v10 practice-log, and schema-v11 acquisition/mastery changes. Source mode is current and verified at schema v11; rebuild and re-run isolated installed smoke before treating a distributable artifact as current.
...
76. Bounded external lexical overlay: current-card context defines the target word and sense, while only an explicit advanced request may add general English synonym/antonym knowledge. The Prompt limits common candidates and requires differences/examples; the Service adds an unavoidable unreviewed-content label; no generated candidate is persisted, treated as validated aid data, or attributed to personal confusion evidence.
