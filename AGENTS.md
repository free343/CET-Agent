...
- `app/db/models.py` defines Word, LearningState, ReviewLog, FavoriteWord, WordLearningAid, ConfusionEdge, AIAnalysis, EmbeddingCache, StudyLevelActivation, ReminderRuntimeState, and ReminderReviewLease.
...
- The main sidebar uses an explicit deep-indigo background, near-white navigation text, and high-contrast hover/selected states. The favorite action, learning-aid panel, and wordbook list also have explicit theme selectors instead of relying on platform defaults.
...
- `app/services/learning_aid_view.py` deterministically maps stored JSON into bounded display strings (`英文搭配｜中文义`, `单词 (词性)｜中文义`) and resolves examples so curated words keep `Word.example` while open words use the validated aid example. `review_service.py` and `wordbook_service.py` consume it; the review card, review page, and wordbook page only present the resulting strings and never parse raw JSON, query SQLAlchemy, or call a model adapter. Multiple collocations or word-family entries are rendered one per line for scanability.
...
|---|---:|---:|
| Full pytest suite | 280 passed in 38.55s | 2026-08-23 |
...
| Ruff format gate | 109 files already formatted | 2026-08-23 |
| Mypy application/scripts check | exit code 0; 68 source files | 2026-08-23 |
...
| Schema migration matrix | 11 focused tests passed: fresh, pre-version adoption, v1→v2 FSRS mapping, v2→v3 level activation, v3→v4 review leases, v4→v5 demo-state repair plus successful four-choice rating persistence, v5→v6 undo snapshots, v6→v7 favorite wordbook, v7→v8 word learning aids, idempotency/newer-version guard, transactional rollback | 2026-08-23 |
...
| Offscreen startup/shutdown smoke | schema-v8 run exited 0 through the real deferred-close path and imported 4,611 validated learning-aid rows | 2026-08-23 |
...
| Multi-item learning-aid layout | Review-card regression confirms multiple collocations/word-family entries render as newline-separated lines rather than middle-dot separators | 2026-08-23 |
| SQLite integrity and foreign keys | current source database is schema v8 with 4,611 learning-aid rows; `integrity_check=ok`; `foreign_key_check` returned 0 violations | 2026-08-23 |
...
- No reproducible source defect is open after the generated-learning-aid hardening pass. The complete 4,611-record artifact passed the deterministic validator and was imported successfully; semantic quality remains an AI-generated-content review concern rather than a persistence or scheduling defect.
...
- The complete artifact contains 740 entries with an intentionally empty word-family list; this is allowed by the “quality over quantity” contract. Collocations and examples are present and validated structurally, but semantic commonness, translation quality, and usefulness still require sampled human review.
...
- The last distributable onedir package/installer remains historical and predates the current review-UX, wordbook, and generated-aid changes. Source mode is current and verified at schema v8; rebuild and re-run isolated installed smoke before treating a distributable artifact as current.
