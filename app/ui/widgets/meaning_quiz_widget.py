"""Reusable presentation widget for deterministic meaning choices."""
...
from app.domain.meaning_quiz import MeaningOption


class MeaningQuizWidget(QFrame):
    option_selected = Signal(int)

    def __init__(self) -> None:
        super().__init__()
        self.options: tuple[MeaningOption, ...] = ()
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 8, 0, 4)
        layout.setSpacing(8)
        self.help_label = QLabel("选择最符合的中文释义，也可以先主动回忆。")
    ...
    def set_options(self, options: tuple[MeaningOption, ...]) -> bool:
        self.reset()
        if len(options) != 4:
            return False
        ...
        for index, (button, option) in enumerate(
            ...
            button.setText(self._choice_label(index, option.meaning))
            button.setEnabled(True)
        self.show()
        return True

    def option(self, index: int) -> MeaningOption | None:
    ...
    def reset(self) -> None:
        self.options = ()
        self.help_label.setText("选择最符合的中文释义，也可以先主动回忆。")
    ...
    @staticmethod
    def _choice_label(index: int, meaning: str) -> str:
        compact = " ".join(meaning.split())
        if len(compact) > 90:
