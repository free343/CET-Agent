...
逐词字段级词形候选 JSONL（仅候选，不进入应用）
    ↓
WordNet/COW 单义项关系候选 JSONL（仅候选，不进入应用）
    ↓
词性/义项/重复/常用度过滤
    ↓
分层人工抽检与问题回流
...
关系候选试验工件（`data/word_lexical_relation_candidates.jsonl`）同样覆盖全部 4,611
个头词，但只有 1,223 个词通过“恰好一个中文重合义项、词库内单词目标、ECDICT 频率
大于 0、词性兼容”的首轮筛选；542 个词因多个对齐义项暂缓，2,846 个词暂无可用对齐
义项。试验共保留 1,237 个关系组（1,125 近义、112 反义）和 1,777 个关系目标，
每个目标带 OEWN、COW、ECDICT 三条字段证据。该覆盖低于未过滤审计的 1,629 个单义
候选，是有意的精度优先结果，不应通过放宽筛选来追求数量。
...
3. **已完成：关系候选试验。** `scripts/generate_lexical_relation_candidates.py` 与独立验证器只保留 1,223 个首轮单义项词；被多个义项或频率/词性门禁排除的记录仍保留明确状态，不会伪装成空事实。
4. **下一步：分层人工审核。** 按 CET4/CET6、词性、冲突类别、频率和规则/不规则样本审核词形候选；对 1,223 个关系词按近义/反义、单义/多义和目标频率抽样，形成错误、合法变体、词义疑点和可晋升清单。
